# 开发指南

## 🛠️ 环境准备

### 系统要求

- Node.js >= 16.0.0
- pnpm >= 8.0.0
- Git

### 安装依赖

```bash
# 安装 pnpm (如果未安装)
npm install -g pnpm

# 克隆项目
git clone https://github.com/your-repo/vue3-webpack.git
cd vue3-webpack

# 安装依赖
pnpm install
```

### 环境变量配置

```bash
# 复制环境变量模板
cp .env.example .env.local

# 编辑 .env.local
VITE_AUTH_SERVER_URL=https://auth.xxx.com
VITE_CLIENT_ID=your-client-id
VITE_API_BASE_URL=https://api.xxx.com
```

## 🚀 本地开发

### 端口规划

| 应用            | 端口 | 访问地址              |
| --------------- | ---- | --------------------- |
| 主应用(Root)    | 3000 | http://localhost:3000 |
| Account(账号)   | 3001 | http://localhost:3001 |
| Designer        | 3002 | http://localhost:3002 |
| Renderer        | 3003 | http://localhost:3003 |
| Admin           | 3004 | http://localhost:3004 |
| Auth 文档       | 3005 | http://localhost:3005 |
| UI 文档(PC)     | 3006 | http://localhost:3006 |
| UI 文档(Mobile) | 3007 | http://localhost:3007 |
| Tools 文档      | 3008 | http://localhost:3008 |

### 启动开发服务器

```bash
# 启动主应用
pnpm run dev

# 启动子应用
pnpm --filter account dev
pnpm --filter designer dev
pnpm --filter renderer dev
pnpm --filter admin dev

# 同时启动多个应用(使用 concurrently)
pnpm run dev:all
```

### 开发新应用

```bash
# 1. 创建新应用目录
mkdir -p @cwj/your-app

# 2. 初始化 package.json
cd @cwj/your-app
pnpm init

# 3. 安装依赖
pnpm add vue vue-router pinia @cwj/auth @cwj/ui-pc @cwj/tools

# 4. 创建基础结构
mkdir -p src/{components,views,router,store,api,utils,types,assets}

# 5. 创建入口文件 src/main.ts
# 6. 配置 webpack.config.ts
# 7. 配置 tsconfig.json
```

### 开发新的 npm 包

```bash
# 1. 创建包目录
mkdir -p @cwj/your-package

# 2. 初始化
cd @cwj/your-package
pnpm init

# 3. 配置 package.json
{
  "name": "@cwj/your-package",
  "version": "1.0.0",
  "type": "module",
  "main": "./dist/index.cjs",
  "module": "./dist/index.mjs",
  "types": "./dist/index.d.ts",
  "exports": {
    ".": {
      "import": "./dist/index.mjs",
      "require": "./dist/index.cjs",
      "types": "./dist/index.d.ts"
    }
  },
  "scripts": {
    "build": "tsup src/index.ts --format esm,cjs --dts",
    "dev": "tsup src/index.ts --format esm,cjs --dts --watch"
  }
}

# 4. 安装构建工具
pnpm add -D tsup typescript

# 5. 创建 src/index.ts
# 6. 开发和构建
pnpm run dev    # 开发模式
pnpm run build  # 构建
```

## 🔧 开发命令

### 常用命令

```bash
# 开发
pnpm run dev              # 启动主应用
pnpm run dev:micro        # 启动微应用
pnpm run dev:all          # 启动所有应用

# 构建
pnpm run build            # 构建主应用
pnpm run build:all        # 构建所有应用
pnpm --filter <app> build # 构建指定应用

# 代码质量
pnpm run type-check       # TypeScript 类型检查
pnpm run lint             # ESLint 代码检查
pnpm run lint:fix         # 自动修复 ESLint 问题
pnpm run format           # Prettier 格式化代码
pnpm run format:check     # 检查代码格式
pnpm run check            # 完整检查(类型+lint+格式)

# 测试
pnpm run test             # 运行测试
pnpm run test:watch       # 监听模式运行测试
pnpm run test:coverage    # 生成测试覆盖率报告

# 包管理
pnpm add <package>        # 添加依赖到根项目
pnpm --filter <app> add <package>  # 添加依赖到指定应用
pnpm update               # 更新所有依赖
```

### Workspace 命令

```bash
# 在所有包中执行命令
pnpm -r run build         # 递归构建所有包
pnpm -r run test          # 递归测试所有包

# 在指定包中执行命令
pnpm --filter designer dev
pnpm --filter @cwj/auth build

# 查看依赖关系
pnpm list                 # 查看所有依赖
pnpm why <package>        # 查看为什么安装了某个包
```

## 🐛 调试

### VSCode 调试配置

创建 `.vscode/launch.json`:

```json
{
  "version": "0.2.0",
  "configurations": [
    {
      "type": "chrome",
      "request": "launch",
      "name": "调试主应用",
      "url": "http://localhost:3000",
      "webRoot": "${workspaceFolder}/src",
      "sourceMapPathOverrides": {
        "webpack:///./*": "${webRoot}/*"
      }
    },
    {
      "type": "chrome",
      "request": "launch",
      "name": "调试设计器",
      "url": "http://localhost:3002",
      "webRoot": "${workspaceFolder}/@cwj/designer/src"
    }
  ]
}
```

### 浏览器调试

1. 打开浏览器开发者工具
2. 在 Sources 面板中找到源代码
3. 设置断点
4. 刷新页面触发断点

### Vue Devtools

安装 Vue Devtools 浏览器扩展:

- Chrome: https://chrome.google.com/webstore
- Firefox: https://addons.mozilla.org/

## 📝 代码提示

### VSCode 插件推荐

在 `.vscode/extensions.json` 中配置:

```json
{
  "recommendations": [
    "vue.volar",
    "dbaeumer.vscode-eslint",
    "esbenp.prettier-vscode",
    "lokalise.i18n-ally",
    "antfu.iconify"
  ]
}
```

### TypeScript 类型提示

```typescript
// 在 src/shims-vue.d.ts 中声明模块类型
declare module '*.vue' {
  import type { DefineComponent } from 'vue'
  const component: DefineComponent<{}, {}, any>
  export default component
}

// 使用类型导入
import type { Ref } from 'vue'
import type { RouteLocationNormalized } from 'vue-router'
```

## 🔍 常见问题

### 1. 端口被占用

```bash
# 查找占用端口的进程
lsof -i :3000

# 杀死进程
kill -9 <PID>

# 或者修改端口
# 在 webpack.config.ts 中修改 devServer.port
```

### 2. 依赖安装失败

```bash
# 清理缓存
pnpm store prune

# 删除 node_modules 和 lock 文件
rm -rf node_modules pnpm-lock.yaml

# 重新安装
pnpm install
```

### 3. 类型检查错误

```bash
# 清理 TypeScript 缓存
rm -rf node_modules/.cache

# 重新生成类型
pnpm run type-check
```

### 4. 热更新不生效

```bash
# 检查 webpack.config.ts 中的 HMR 配置
devServer: {
  hot: true,
  liveReload: true
}

# 检查文件监听数量限制(Linux/Mac)
echo fs.inotify.max_user_watches=524288 | sudo tee -a /etc/sysctl.conf
sudo sysctl -p
```

### 5. Module Federation 加载失败

```bash
# 检查 remoteEntry.js 是否可访问
curl http://localhost:3001/remoteEntry.js

# 检查 webpack.config.ts 中的 Module Federation 配置
# 确保 name 和 remotes 配置正确
```

### 6. Cookie 不共享

```bash
# 检查 Cookie 配置
# domain 必须设置为 .xxx.com
# 确保所有应用使用相同的域名

# 检查浏览器控制台
# Application -> Cookies -> 查看 Cookie 配置
```

## 🎯 开发技巧

### 1. 使用代码片段

在 `.vscode/snippets.code-snippets` 中创建代码片段:

```json
{
  "Vue Component": {
    "prefix": "vcomp",
    "body": [
      "<template>",
      "  <div class=\"${1:component-name}\">",
      "    ${2}",
      "  </div>",
      "</template>",
      "",
      "<script setup lang=\"ts\">",
      "${3}",
      "</script>",
      "",
      "<style scoped>",
      ".${1:component-name} {",
      "  ${4}",
      "}",
      "</style>"
    ]
  }
}
```

### 2. 使用 Git Hooks

```bash
# 安装 husky
pnpm add -D husky

# 初始化
npx husky install

# 添加 pre-commit hook
npx husky add .husky/pre-commit "pnpm run lint"
npx husky add .husky/pre-commit "pnpm run type-check"
```

### 3. 使用 Mock 数据

```typescript
// src/mock/index.ts
import { setupWorker } from 'msw'
import { handlers } from './handlers'

export const worker = setupWorker(...handlers)

// 在开发环境启动
if (import.meta.env.DEV) {
  worker.start()
}
```

### 4. 性能分析

```bash
# 使用 webpack-bundle-analyzer
pnpm add -D webpack-bundle-analyzer

# 在 webpack.config.ts 中添加
const { BundleAnalyzerPlugin } = require('webpack-bundle-analyzer')

plugins: [
  new BundleAnalyzerPlugin()
]

# 构建并分析
pnpm run build
```

## 📚 相关资源

- [Vue 3 文档](https://cn.vuejs.org/)
- [TypeScript 文档](https://www.typescriptlang.org/)
- [Webpack 文档](https://webpack.js.org/)
- [Module Federation 文档](https://webpack.js.org/concepts/module-federation/)
- [Pinia 文档](https://pinia.vuejs.org/)
- [Element Plus 文档](https://element-plus.org/)
