# @cwj/auth 认证包文档

## 📖 概述

`@cwj/auth` 是一个轻量级的认证授权包,为微前端应用提供统一的认证能力。

### 核心特性

- ✅ **Token 管理**: Cookie 存储,利用浏览器同源策略自动携带
- ✅ **URL 拦截**: 页面加载时检测 Token,无 Token 自动跳转登录
- ✅ **XHR 拦截**: 统一拦截所有 HTTP 请求,处理业务状态码
- ✅ **权限控制**: 提供路由级和按钮级权限判断
- ✅ **框架无关**: 核心功能不依赖 Vue Router,可在任何框架中使用
- ✅ **TypeScript**: 完整的类型定义

## 📦 安装

```bash
# 使用 pnpm
pnpm add @cwj/auth

# 使用 npm
npm install @cwj/auth

# 使用 yarn
yarn add @cwj/auth
```

## 🚀 快速开始

### 基础配置

```typescript
// main.ts
import { createApp } from 'vue'
import { createAuth, setupUrlGuard, setupXhrInterceptor } from '@cwj/auth'
import App from './App.vue'

const app = createApp(App)

// 1. 创建认证实例
const auth = createAuth({
  loginUrl: 'https://xx.xxx.com/account/',
  storage: 'cookie',
  tokenKey: 'access_token',
  cookieOptions: {
    domain: '.xxx.com',
    path: '/',
    secure: true,
    sameSite: 'Lax',
  },
})

app.use(auth)

// 2. 初始化 URL 拦截器(页面加载时检测 Token)
setupUrlGuard({
  loginUrl: 'https://xx.xxx.com/account/',
  whitelist: ['/about', '/help'],
})

// 3. 初始化 XHR 拦截器(拦截所有 HTTP 请求)
setupXhrInterceptor({
  loginUrl: 'https://xx.xxx.com/account/',
  tokenExpiredCode: 10001,
})

app.mount('#app')
```

## 📚 API 文档

### createAuth()

创建认证实例并注册为 Vue 插件。

**类型签名:**

```typescript
function createAuth(options: AuthOptions): Plugin
```

**参数:**

```typescript
interface AuthOptions {
  loginUrl: string              // 登录页地址
  storage: 'cookie'             // 存储方式
  tokenKey?: string             // Token 键名,默认 'access_token'
  cookieOptions?: CookieOptions // Cookie 配置
}

interface CookieOptions {
  domain?: string    // Cookie 域名,如 '.xxx.com'
  path?: string      // Cookie 路径,默认 '/'
  secure?: boolean   // 是否仅 HTTPS,默认 true
  sameSite?: 'Strict' | 'Lax' | 'None'  // 默认 'Lax'
  maxAge?: number    // 过期时间(秒)
}
```

**示例:**

```typescript
const auth = createAuth({
  loginUrl: 'https://xx.xxx.com/account/',
  storage: 'cookie',
  tokenKey: 'access_token',
  cookieOptions: {
    domain: '.xxx.com',
    path: '/',
    secure: true,
    sameSite: 'Lax',
  },
})

app.use(auth)
```

### useAuth()

获取认证状态和方法的 Composable。

**类型签名:**

```typescript
function useAuth(): {
  token: Ref<string | null>
  isAuthenticated: ComputedRef<boolean>
  userInfo: Ref<UserInfo | null>
  setToken: (token: string) => void
  getToken: () => string | null
  clearToken: () => void
  logout: () => void
  redirectToLogin: (redirect?: string) => void
}
```

**返回值:**

| 属性 | 类型 | 说明 |
|------|------|------|
| `token` | `Ref<string \| null>` | Token 响应式引用 |
| `isAuthenticated` | `ComputedRef<boolean>` | 是否已认证 |
| `userInfo` | `Ref<UserInfo \| null>` | 用户信息 |
| `setToken` | `(token: string) => void` | 设置 Token |
| `getToken` | `() => string \| null` | 获取 Token |
| `clearToken` | `() => void` | 清除 Token |
| `logout` | `() => void` | 登出(清除 Token 并跳转) |
| `redirectToLogin` | `(redirect?: string) => void` | 跳转到登录页 |

**示例:**

```typescript
<script setup lang="ts">
import { useAuth } from '@cwj/auth'

const { token, isAuthenticated, setToken, logout } = useAuth()

// 获取 Token
console.log(token.value)

// 检查是否已登录
if (isAuthenticated.value) {
  console.log('已登录')
}

// 登出
function handleLogout() {
  logout()
}
</script>
```

### setupUrlGuard()

设置 URL 拦截器,在页面加载时检测 Token。

**类型签名:**

```typescript
function setupUrlGuard(options: UrlGuardOptions): void
```

**参数:**

```typescript
interface UrlGuardOptions {
  loginUrl: string           // 登录页地址
  whitelist?: string[]       // 白名单路径
  onNoToken?: () => void     // 无 Token 时的回调
}
```

**示例:**

```typescript
setupUrlGuard({
  loginUrl: 'https://xx.xxx.com/account/',
  whitelist: ['/about', '/help', '/public'],
  onNoToken: () => {
    console.log('未登录,跳转到登录页')
  },
})
```

**工作原理:**

1. 页面加载时自动执行
2. 检查当前路径是否在白名单中
3. 如果不在白名单,检测 Cookie 中的 Token
4. 无 Token 则跳转到登录页,并携带 `redirect` 参数

### setupXhrInterceptor()

设置 XHR 拦截器,拦截所有 HTTP 请求。

**类型签名:**

```typescript
function setupXhrInterceptor(options: XhrInterceptorOptions): void
```

**参数:**

```typescript
interface XhrInterceptorOptions {
  loginUrl: string              // 登录页地址
  tokenExpiredCode?: number     // Token 过期的业务状态码,默认 10001
  onTokenExpired?: () => void   // Token 过期时的回调
}
```

**示例:**

```typescript
setupXhrInterceptor({
  loginUrl: 'https://xx.xxx.com/account/',
  tokenExpiredCode: 10001,
  onTokenExpired: () => {
    console.log('Token 已过期')
    // 可以在这里添加其他逻辑,如清理本地数据
  },
})
```

**工作原理:**

1. 拦截 `XMLHttpRequest` 和 `Fetch API`
2. Cookie 自动携带(浏览器行为,无需手动添加)
3. 响应拦截:检测业务状态码
4. 如果 `code === 10001`,清除 Cookie 并跳转登录

### usePermission()

权限判断 Composable。

**类型签名:**

```typescript
function usePermission(): {
  hasPermission: (permission: string) => boolean
  hasAnyPermission: (permissions: string[]) => boolean
  hasAllPermissions: (permissions: string[]) => boolean
}
```

**返回值:**

| 方法 | 类型 | 说明 |
|------|------|------|
| `hasPermission` | `(permission: string) => boolean` | 检查单个权限 |
| `hasAnyPermission` | `(permissions: string[]) => boolean` | 检查任意权限 |
| `hasAllPermissions` | `(permissions: string[]) => boolean` | 检查所有权限 |

**示例:**

```typescript
<script setup lang="ts">
import { usePermission } from '@cwj/auth'

const { hasPermission, hasAnyPermission } = usePermission()

// 检查单个权限
const canEdit = hasPermission('user.edit')

// 检查任意权限
const canManage = hasAnyPermission(['user.edit', 'user.delete'])
</script>

<template>
  <button v-if="canEdit">编辑</button>
  <button v-if="canManage">管理</button>
</template>
```

### authGuard()

Vue Router 认证守卫(可选)。

**类型签名:**

```typescript
function authGuard(options: AuthGuardOptions): NavigationGuard
```

**参数:**

```typescript
interface AuthGuardOptions {
  loginUrl: string       // 登录页地址
  whitelist?: string[]   // 白名单路由
}
```

**示例:**

```typescript
import { createRouter } from 'vue-router'
import { authGuard } from '@cwj/auth'

const router = createRouter({
  // ... routes
})

router.beforeEach(
  authGuard({
    loginUrl: 'https://xx.xxx.com/account/',
    whitelist: ['/about', '/help'],
  })
)
```

### v-permission 指令

权限控制指令。

**用法:**

```vue
<template>
  <!-- 单个权限 -->
  <button v-permission="'user.edit'">编辑</button>

  <!-- 任意权限 -->
  <button v-permission:any="['user.edit', 'user.delete']">操作</button>

  <!-- 所有权限 -->
  <button v-permission:all="['user.read', 'user.edit']">高级操作</button>
</template>
```

## 🔐 Token 管理

### Cookie 存储

Token 存储在 Cookie 中,利用浏览器的同源策略自动携带。

**优势:**

- ✅ 浏览器自动携带(同源请求)
- ✅ 支持跨子应用共享
- ✅ 可设置 `HttpOnly` 防止 XSS(后端设置)
- ✅ 可设置 `Secure` 仅 HTTPS 传输
- ✅ 可设置 `SameSite` 防止 CSRF

**配置示例:**

```typescript
cookieOptions: {
  domain: '.xxx.com',    // 支持所有子域名
  path: '/',
  secure: true,          // 仅 HTTPS
  sameSite: 'Lax',       // 防止 CSRF
}
```

**域名说明:**

```
domain: '.xxx.com'

支持的域名:
✅ xx.xxx.com (主应用)
✅ xx.xxx.com/designer (设计器)
✅ xx.xxx.com/renderer (渲染器)
✅ xx.xxx.com/admin (管理后台)
✅ xx.xxx.com/account (账号中心)

Cookie 自动共享,无需手动传递
```

### Token 操作

```typescript
import { useAuth } from '@cwj/auth'

const { setToken, getToken, clearToken } = useAuth()

// 设置 Token(登录成功后)
setToken('your-jwt-token')
// 自动写入 Cookie:
// document.cookie = 'access_token=xxx; domain=.xxx.com; path=/; secure; samesite=Lax'

// 获取 Token
const token = getToken()
console.log(token)

// 清除 Token(登出)
clearToken()
// 自动删除 Cookie:
// document.cookie = 'access_token=; domain=.xxx.com; path=/; max-age=0'
```

## 🛡️ 认证流程

### 1. 用户登录(account 站点)

```typescript
// account 站点 - 登录页面
import { useAuth } from '@cwj/auth'

const { setToken } = useAuth()

async function handleLogin() {
  try {
    // 调用登录接口
    const res = await loginApi({
      username: form.username,
      password: form.password,
    })

    // 保存 Token 到 Cookie
    setToken(res.data.token)

    // 跳转回原页面或首页
    const redirect = new URLSearchParams(location.search).get('redirect')
    window.location.href = redirect || '/'
  } catch (error) {
    console.error('登录失败:', error)
  }
}
```

### 2. 主应用/子应用访问

```typescript
// main.ts
import { setupUrlGuard } from '@cwj/auth'

// 初始化 URL 拦截器
setupUrlGuard({
  loginUrl: 'https://xx.xxx.com/account/',
  whitelist: ['/about', '/help'],
})

// 工作流程:
// 1. 页面加载时自动执行
// 2. 检测 Cookie 中的 token
// 3. 如果没有 token,自动跳转到 account
// 4. 如果有 token,正常加载页面
```

### 3. Token 过期处理

```typescript
// main.ts
import { setupXhrInterceptor } from '@cwj/auth'

// 初始化 XHR 拦截器
setupXhrInterceptor({
  loginUrl: 'https://xx.xxx.com/account/',
  tokenExpiredCode: 10001,
})

// 工作流程:
// 1. 拦截所有 HTTP 请求
// 2. Cookie 自动携带(浏览器行为)
// 3. 响应拦截:检测业务状态码
// 4. 如果 code === 10001,清除 Cookie 并跳转登录
```

### 4. 后端接口响应格式

```typescript
// 成功响应
{
  "code": 200,
  "message": "success",
  "data": { ... }
}

// Token 过期(业务状态码 10001)
{
  "code": 10001,
  "message": "Token 已过期",
  "data": null
}
```

## 🎯 使用场景

### 场景 1: 主应用集成

```typescript
// src/main.ts
import { createAuth, setupUrlGuard, setupXhrInterceptor } from '@cwj/auth'

const auth = createAuth({
  loginUrl: 'https://xx.xxx.com/account/',
  storage: 'cookie',
  cookieOptions: {
    domain: '.xxx.com',
    path: '/',
    secure: true,
    sameSite: 'Lax',
  },
})

app.use(auth)

setupUrlGuard({
  loginUrl: 'https://xx.xxx.com/account/',
  whitelist: ['/about'],
})

setupXhrInterceptor({
  loginUrl: 'https://xx.xxx.com/account/',
  tokenExpiredCode: 10001,
})
```

### 场景 2: 子应用集成(独立访问)

```typescript
// @cwj/designer/src/main.ts
import { createAuth, setupUrlGuard, setupXhrInterceptor } from '@cwj/auth'

const auth = createAuth({
  loginUrl: 'https://xx.xxx.com/account/',
  storage: 'cookie',
  cookieOptions: {
    domain: '.xxx.com',
    path: '/',
    secure: true,
    sameSite: 'Lax',
  },
})

app.use(auth)

// 独立访问时:
// 1. URL 拦截器检测 Cookie 中的 token
// 2. 如果没有 token,自动跳转到 account 登录
// 3. 登录成功后,account 设置 Cookie 并跳转回来
// 4. Cookie 自动在所有子应用间共享

setupUrlGuard({
  loginUrl: 'https://xx.xxx.com/account/',
})

setupXhrInterceptor({
  loginUrl: 'https://xx.xxx.com/account/',
  tokenExpiredCode: 10001,
})
```

### 场景 3: 路由级权限控制

```typescript
// router/index.ts
import { createRouter } from 'vue-router'
import { authGuard, permissionGuard } from '@cwj/auth'

const router = createRouter({
  // ... routes
})

// 认证守卫
router.beforeEach(
  authGuard({
    loginUrl: 'https://xx.xxx.com/account/',
    whitelist: ['/about', '/help'],
  })
)

// 权限守卫
router.beforeEach(
  permissionGuard({
    getPermissions: route => route.meta.permissions,
    onDenied: () => {
      router.push('/403')
    },
  })
)
```

### 场景 4: 按钮级权限控制

```vue
<script setup lang="ts">
import { usePermission } from '@cwj/auth'

const { hasPermission, hasAnyPermission } = usePermission()

const canEdit = hasPermission('user.edit')
const canDelete = hasPermission('user.delete')
const canManage = hasAnyPermission(['user.edit', 'user.delete'])
</script>

<template>
  <div>
    <button v-if="canEdit">编辑</button>
    <button v-if="canDelete">删除</button>
    <button v-if="canManage">管理</button>

    <!-- 或使用指令 -->
    <button v-permission="'user.edit'">编辑</button>
    <button v-permission:any="['user.edit', 'user.delete']">操作</button>
  </div>
</template>
```

## 🔧 高级配置

### 自定义 Cookie 配置

```typescript
const auth = createAuth({
  loginUrl: 'https://xx.xxx.com/account/',
  storage: 'cookie',
  tokenKey: 'my_token',  // 自定义 Token 键名
  cookieOptions: {
    domain: '.xxx.com',
    path: '/',
    secure: true,
    sameSite: 'Strict',  // 更严格的 CSRF 防护
    maxAge: 7 * 24 * 60 * 60,  // 7 天过期
  },
})
```

### 自定义 Token 过期处理

```typescript
setupXhrInterceptor({
  loginUrl: 'https://xx.xxx.com/account/',
  tokenExpiredCode: 10001,
  onTokenExpired: () => {
    // 自定义处理逻辑
    console.log('Token 已过期')
    
    // 清理本地数据
    localStorage.clear()
    sessionStorage.clear()
    
    // 显示提示
    ElMessage.warning('登录已过期,请重新登录')
  },
})
```

## 📝 类型定义

```typescript
// 认证配置
interface AuthOptions {
  loginUrl: string
  storage: 'cookie'
  tokenKey?: string
  cookieOptions?: CookieOptions
}

// Cookie 配置
interface CookieOptions {
  domain?: string
  path?: string
  secure?: boolean
  sameSite?: 'Strict' | 'Lax' | 'None'
  maxAge?: number
}

// 用户信息
interface UserInfo {
  id: number
  name: string
  email: string
  avatar?: string
  roles?: string[]
  permissions?: string[]
}

// URL 拦截器配置
interface UrlGuardOptions {
  loginUrl: string
  whitelist?: string[]
  onNoToken?: () => void
}

// XHR 拦截器配置
interface XhrInterceptorOptions {
  loginUrl: string
  tokenExpiredCode?: number
  onTokenExpired?: () => void
}
```

## ❓ 常见问题

### 1. Cookie 不共享?

**检查 Cookie 配置:**

```typescript
cookieOptions: {
  domain: '.xxx.com',  // 必须设置为 .xxx.com
  path: '/',
}
```

**检查浏览器控制台:**

打开 `Application -> Cookies` 查看 Cookie 配置。

### 2. Token 过期后没有跳转?

**检查后端响应格式:**

```json
{
  "code": 10001,  // 必须是业务状态码,不是 HTTP 状态码
  "message": "Token 已过期"
}
```

**检查拦截器配置:**

```typescript
setupXhrInterceptor({
  tokenExpiredCode: 10001,  // 确保与后端一致
})
```

### 3. 白名单不生效?

**检查路径匹配:**

```typescript
setupUrlGuard({
  whitelist: ['/about', '/help'],  // 使用 startsWith 匹配
})

// 匹配规则:
// ✅ /about -> 匹配
// ✅ /about/detail -> 匹配
// ❌ /user/about -> 不匹配
```

## 📚 相关资源

- [项目文档](../../README.md)
- [架构设计](../架构设计.md)
- [开发指南](../开发指南.md)
- [部署文档](../部署文档.md)

## 📄 License

MIT

