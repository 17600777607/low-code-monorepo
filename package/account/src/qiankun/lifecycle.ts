...
function bootstrap() {
  ...
  console.groupCollapsed('📦 [account] 子应用内部生命周期')
  consola.ready('[account] bootstrap')
}
...
function unmount(props: QiankunProps) {
  console.groupCollapsed('📦 [account] 子应用卸载')
  consola.info('[account] unmount', props)
  if (app) {
    app.unmount()
    app = null
  }
...
function update(props: QiankunProps) {
  consola.log('[account] update', props)
}
